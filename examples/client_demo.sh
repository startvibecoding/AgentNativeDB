...
echo "2. 创建会话..."
./bin/client -server localhost:8400 create-session demo-agent-001
...
if [ -n "$SESSION_ID" ]; then
    ./bin/client -server localhost:8400 store-memory "$SESSION_ID" short_term "用户喜欢简洁的界面" 0.8
    ./bin/client -server localhost:8400 store-memory "$SESSION_ID" long_term "用户是高级用户，喜欢命令行工具" 0.9
fi
...
echo "8. 导出数据..."
./bin/client -server localhost:8400 export sessions /tmp/demo_sessions.json
